...
process_request() {
  ...
  # # NOTE: This may be obsolete now, since we let haserl process request
  # # body directly from stdin (from socat fifo).
  # #
  # # This could be made into a function, then called only when necessary.
  # if [ $(($CONTENT_LENGTH)) -gt 0 ]; then
  #   log 6 '-echo "Calling head on stdin with -c $CONTENT_LENGTH"'
  #   export REQUEST_BODY="$( head -c $(($CONTENT_LENGTH)) )"
  # fi
  #
  # # This version is from handle_scgi().
...
eval_haserl_env() {
  {
    log 5 '-echo "Evaling current env with haserl template $HASERL_ENV ($(get_pids))"'
    #log 6 '-echo "Sending REQUEST_BODY to haserl stdin: $REQUEST_BODY"'
    #haserl_env=$(printf '%s' "$REQUEST_BODY" | haserl "$HASERL_ENV")
    # Does haserl know to read request body from stdin? Yes, it does, this works!
    haserl_env=$(haserl "$HASERL_ENV")
